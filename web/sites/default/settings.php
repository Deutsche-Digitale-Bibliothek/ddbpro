...
$settings['update_free_access'] = $env_bool('UPDATE_FREE_ACCESS');
...
$settings['trusted_host_patterns'] = preg_split(
  '/,\s*/',
...
// Redis is disabled during installation because its module services are not
// available in the installer container yet.
$use_redis = $env_bool('USE_REDIS');
if ($use_redis && !\Drupal\Core\Installer\InstallerKernel::installationAttempted()) {
  if (!extension_loaded('redis')) {
    throw new RuntimeException('USE_REDIS is enabled, but the PHP Redis extension is not loaded.');
  }

  // Load the base services explicitly so this remains safe if the Drupal Redis
  // module is temporarily disabled in configuration.
